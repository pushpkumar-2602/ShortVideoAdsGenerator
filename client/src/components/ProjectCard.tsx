          ...
          <div
            ...
            {!forCommunity && (
                ...
                className="absolute right-3 top-3 sm:opacity-0 group-hover:opacity-100 transition flex items-center gap-2">
                    ...
                    <div className="flex flex-col items-end w-32 text-sm">
                        <ul className={`text-xs ${menuOpen ? 'block' : 'hidden'} overflow-hidden right-0 peer-focus:block hover:block w-40 bg-black/50 backdrop-blur 
                            text-white border border-gray-500/50 rounded-lg shadow-md mt-2 py-1 z-10`}>
                            {gen.generatedImage && < a href='#' download className="flex gap-2 items-cente
                            r px-4 py-2 hover:bg-black/10 cursor-pointer"> 
                            <ImageIcon size={14}/>Download Image</a>}

                            {gen.generatedVideo && < a href='#' download className="flex gap-2 items-cente
                            r px-4 py-2 hover:bg-black/10 cursor-pointer"> 
                            <PlaySquareIcon size={14}/>Download Video</a>}

                            {(gen.generatedVideo || gen.generatedImage) && <button
                            onClick={()=>navigator.share({url:gen.generatedVideo || gen.generatedImage, title:gen.productName,
            ...
            {/*Source Images */}
             <div className='absolute right-3 bottom-3'>
                ...
                <img src={gen.uploadedImages[1]} alt="product" className="w-16 h-16 object-cover rounded-full animate-float  -ml-8" 
          ...
          <div className='p-4'>
             ...
             {gen.productDescription && (
                <div className='mt-3'>
                    ...
                    <div className="text-sm text-gray-300 bg-white/5 p-2 rounded-md wrap-
                    break-words">{gen.productDescription}</div>
                </div>
             )}
             {/*User Prompt */}
             {gen.userPrompt && (
